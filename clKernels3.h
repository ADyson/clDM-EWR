...
"__kernel void clPadCrop(__global const float* restrict Input, __global float2* restrict Output, int fullwidth, int fullheight, int padLeft, int padRight, int padTop, int padBottom, float xShift, float yShift, int newwidth, int newheight, int iTop, int iLeft) \n"
"{ \n"
"	//Get the work items ID \n"
"	int xid = get_global_id(0);	\n"
"	int yid = get_global_id(1); \n"
...
"			Output[Index] = 0; \n"
"		} else { \n"
"			float v1 = (1-subx)*(1-suby)*Input[iLeft + xs2 + xid + (iTop+ys2+yid)*fullwidth]; \n"
"			float v2 = (subx)*(1-suby)*Input[iLeft + xs2 + xid + 1 + (iTop+ys2+yid)*fullwidth]; \n"
"			float v3 = (1-subx)*(suby)*Input[iLeft + xs2 + xid + (iTop+ys2+yid + 1)*fullwidth]; \n"
"			float v4 = (subx)*(suby)*Input[iLeft + xs2 + xid + 1 +(iTop+ys2+yid + 1)*fullwidth]; \n"
...
"__kernel void clRotScale(__global const float* Input, __global float* Output, int width, int height, float magscale, float rotscale, float df) \n"
"{ \n"
"	int xid = get_global_id(0);	\n"
"	int yid = get_global_id(1); \n"
"	int centrex = width/2;	\n"
...
"			float subx = newx - xs2; \n"
"			float suby = newy - ys2; \n"
"			float v1 = (1-subx)*(1-suby)*Input[xs2 + ys2*width]; \n"
"			float v2 = (subx)*(1-suby)*Input[xs2 + 1 + ys2*width]; \n"
"			float v3 = (1-subx)*(suby)*Input[xs2 + (ys2 + 1)*width]; \n"
"			float v4 = (subx)*(suby)*Input[xs2 + 1 + (ys2 + 1)*width]; \n"
"			Output[Index] = v1 + v2 + v3 + v4; \n"
"		} \n"
"	}	\n"
"}	\n"
;
